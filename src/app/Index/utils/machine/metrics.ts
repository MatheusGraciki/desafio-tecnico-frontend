...
export function getLastMachineData(machine: Machine): MachineData | undefined {
	return machine.dados?.[machine.dados.length - 1];
}

export function countMachinesByStatus(machines: Machine[]): MachineStatusCount {
	return machines.reduce<MachineStatusCount>(
		(acc, machine) => {
